...
DEFINE_VISIT(SymbolResolver, ThisExpr) {
  shared_ptr<Symbol> currentClassSymbol =
      symbolTable.getCurrentScope()->getCurrentClassSymbol();
  if (currentClassSymbol == nullptr &&
      symbolTable.getCurrentScope()->inFunctionScope()) {
  ...
  ClassType *classType =
      dyn_cast<ClassType>(currentClassSymbol->getType().get());
  if (classType == nullptr) {
    ErrorReporter::reportError(&expr, "Class '" + classType->getName() +
...
DEFINE_VISIT(SymbolResolver, SuperExpr) {
  ...
  if (currentClassSymbol == nullptr &&
    ...
    ErrorReporter::reportError(
        &expr, "'super' can only be used inside a class method");
    return;
  }
...
DEFINE_VISIT(SymbolResolver, CallExpr) {
  ...
  if (auto functionType = dyn_cast<FunctionType>(calleeType)) {
    // If the callee is a function type, set the call expression type as the
...
DEFINE_VISIT(SymbolResolver, VariableExpr) {
  ...
  if (!symbol->isDefinedSymbol()) {
    ...
    ErrorReporter::reportError(&expr, "Variable '" + expr.getName() +
...
DEFINE_VISIT(SymbolResolver, AccessExpr) {
  ...
  if (auto classType = dyn_cast<ClassType>(baseType)) {
    ...
    shared_ptr<Symbol> propertySymbol =
        classType->getProperty(expr.getProperty());
    if (propertySymbol == nullptr) {
      ErrorReporter::reportError(&expr, "Property '" + expr.getProperty() +
                                            "' is not defined in class '" +
                                            classType->getName() + "'");
      return;
    }

    shared_ptr<Type> propertyType =
        classType->getProperty(expr.getProperty())->getType();
    ...
    if (isa<ConstructorType>(propertyType)) {
      ErrorReporter::reportError(
          &expr, "Cannot access constructor function as a property");
      expr.setType(UnresolvedType::getInstance());
      return;
    }
...
DEFINE_VISIT(SymbolResolver, ClassDeclStmt) {
  ...
  shared_ptr<ClassType> classType =
      make_shared<ClassType>(expr.getName(), superClassType);
  ...
  if (!classType->hasConstructor()) {
    ...
    shared_ptr<ConstructorType> constructorType = make_shared<ConstructorType>(
        classSymbol->getName(), classType->getInstanceType());
    shared_ptr<Symbol> constructorSymbol =
        make_shared<Symbol>(classSymbol->getName(), constructorType);
...
DEFINE_VISIT(SymbolResolver, FunctionDecl) {
  ...
  if (symbol == nullptr) {
    ...
    if (isConstructor) {
      ...
      funcType = make_shared<ConstructorType>(
          functionName,
    ...
    ErrorReporter::reportError(&expr, "Symbol '" + functionName +
                                          "' is not a function");
    return;
  }
  ...
  for (const auto &param : expr.getParameters()) {
    ...
    shared_ptr<Symbol> paramSymbol =
        make_shared<Symbol>(param->getName(), UnresolvedType::getInstance());
    paramSymbol->markAsDefined();

    // Declare the parameter in the current scope
    if (!symbolTable.declare(paramSymbol)) {
      ErrorReporter::reportError(&expr, "Parameter '" + param->getName() +
                                            "' is already defined");
      return;
    }
...
DEFINE_VISIT(SymbolResolver, IfStmt) {
  ...
  // expr.setType(NilType::getInstance()); // For now, we assume the if
...
DEFINE_VISIT(SymbolResolver, WhileStmt) {
  ...
  // expr.setType(NilType::getInstance()); // For now, we assume the while loop
  // returns nil
}
...
DEFINE_VISIT(SymbolResolver, ForStmt) {
  ...
  // expr.setType(NilType::getInstance()); // For now, we assume the for loop
  // returns nil
}
...
DEFINE_VISIT(SymbolResolver, ReturnStmt) {
  if (!symbolTable.getCurrentScope()->inFunctionScope()) {
    ErrorReporter::reportError(
        &expr, "Return statement is not allowed outside a function");
    return;
  }

  shared_ptr<FunctionType> currentFunctionType =
      symbolTable.getCurrentScope()->getCurrentFunctionType();

  // check if we are in a constructor
  if (isa<ConstructorType>(currentFunctionType)) {
    ErrorReporter::reportError(
        &expr, "Return statement is not allowed in a constructor");
    return;
  }
