...
namespace lox
{
    ...
    public:
        ...
        Token(TokenType tokenType, const char *start, const char *end, Location location)
            : Token(tokenType, start, end - start, location) {};
        Token(TokenType tokenType, const char *start, const char *end, int line, int column)
            : Token(tokenType, start, end - start, Location(line, column)) {};
        Token(TokenType tokenType, const char *start, int length, int line, int column)
            : Token(tokenType, std::string_view(start, length), Location(line, column)) {};
        ...
        virtual ~Token() = default; // Add a virtual destructor to make Token polymorphic
        ...
        bool operator==(lox::TokenType tokenType) const
        {
        ...
        {
            return type != token.type || tokenStr.compare(token.tokenStr) != 0 || location != token.location;
        }

        const std::string_view &getTokenString() {
            return tokenStr;
        }
    ...
    protected:
        ...
        Token(TokenType tokenType, std::string_view tokenStr, Location loc, const std::optional<std::string> &errMsg)
            : type(tokenType), tokenStr(tokenStr), location(loc), errorMsg(errMsg) {
                assert(tokenType != TokenType::TOKEN_ERROR  || (tokenType == TokenType::TOKEN_ERROR && errMsg.has_value()));
            };
    };
    ...
    public:
        ...
        TokenError(const std::string_view tokenStr, const std::optional<std::string> &errorMsg, Location loc)
            : Token(TokenType::TOKEN_ERROR, tokenStr, loc, errorMsg) {};
        TokenError(const Token &token)
            : Token(token) {
                assert(type == TokenType::TOKEN_ERROR);
            };
        TokenError(const Token &token, const std::string &errorMsg)
            : Token(token) {
