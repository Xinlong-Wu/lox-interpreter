...
// Walker回调函数类型定义
template <typename T> using WalkCallback = std::function<WalkResult(T *)>;

// 无返回值的回调函数类型
template <typename T> using VoidWalkCallback = std::function<void(T *)>;

// Walker类，用于存储和调用不同类型的回调函数
class Walker {
private:
  // 使用type_info作为key来存储不同类型的回调函数
  std::unordered_map<std::type_index, std::function<WalkResult(ASTNode *)>>
      callbacks;
  WalkOrder order;
...
public:
  ...
  auto registerCallback(CallBack &&callback) -> std::enable_if_t<
      std::is_same_v<std::invoke_result_t<CallBack, T *>, WalkResult>, void> {
  ...
  template <typename T, typename CallBack>
  auto registerCallback(CallBack &&callback) -> std::enable_if_t<
      std::is_same_v<std::invoke_result_t<CallBack, T *>, void>, void> {
    callbacks[typeid(T)] = [callback](ASTNode *node) -> WalkResult {
      // if (auto* typed_node = dynamic_cast<T*>(node)) {
      if (auto *typed_node = dyn_cast<T>(node)) {
        callback(typed_node);
  ...
  // 检查是否有特定类型的回调
  template <typename T> bool hasCallback() const {
